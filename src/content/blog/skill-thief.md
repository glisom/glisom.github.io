...
summary: A Claude Code plugin that reads someone else's agent tooling, pulls out the mechanisms worth having, and decides where each one actually belongs in your setup.
draft: false
hasDetailPage: true
featured: false
tags:
...
## Every idea gets exactly one verdict {#every-idea-gets-exactly-one-verdict}
...
## Mechanisms, not features {#mechanisms-not-features}
...
## It's not an installer {#its-not-an-installer}
