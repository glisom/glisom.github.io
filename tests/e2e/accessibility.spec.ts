...
for (const path of representativePages) {
  test(`axe finds no A/AA or best-practice violations on ${path}`, async ({ page }) => {
...
test('keyboard order reaches skip link, identity, and primary content in document order', async ({ page }) => {
  await page.goto('/');
...
test('touch-width controls expose labels without hover', async ({ page }) => {
  ...
  for (const path of ['/', '/blog/', '/listwithme/']) {
    await page.goto(path);
    const names = await page.locator('a:visible, button:visible, summary:visible').evaluateAll((nodes) =>
      nodes.map((node) => (node.getAttribute('aria-label') || node.textContent || '').trim()),
    );
    expect(names.length, path).toBeGreaterThan(0);
    expect(names.every(Boolean), `${path}: ${JSON.stringify(names)}`).toBe(true);
  }
});

test('every phone control, including prose links, renders a 44px hit rectangle', async ({ page }) => {
  test.skip(test.info().project.name !== 'phone');
  const paths = routeFixture.routes
    .filter(({ kind, canonicalPath }) => ['page', 'post'].includes(kind) && canonicalPath !== '/404.html')
    .map(({ canonicalPath }) => canonicalPath);
  const failures: string[] = [];
  for (const path of paths) {
    await interceptThirdParties(page);
    await page.goto(path);
    const undersized = await page.locator('a, button, summary, input, select, textarea').evaluateAll((nodes) =>
      nodes.flatMap((node) => {
        const style = getComputedStyle(node);
...
test('reduced motion computes zero durations and disables smooth scrolling', async ({ page }) => {
  ...
  for (const path of ['/', '/2019/05/30/listwithme.html']) {
    ...
    expect(failures, failures.join('\n')).toEqual([]);
    expect(await page.locator('html').evaluate((node) => getComputedStyle(node).scrollBehavior)).not.toBe('smooth');
  }
});

test('comments request is lazy and occurs exactly once near the comments region', async ({ page }) => {
  ...
  await expect(page.locator('[data-utterances-mount] script[src="https://utteranc.es/client.js"]')).toHaveCount(0);
  await page.locator('[data-comments]').scrollIntoViewIfNeeded();
  await expect.poll(() => requests.length).toBe(1);
  await expect(page.locator('[data-utterances-mount] script[src="https://utteranc.es/client.js"]')).toHaveCount(1);
  await page.locator('[data-comments]').scrollIntoViewIfNeeded();
